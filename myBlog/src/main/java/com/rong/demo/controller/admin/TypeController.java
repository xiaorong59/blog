...
public class TypeController {
    ...
    @GetMapping("/types")
    public String types(@PageableDefault(size = 7, sort = {"id"}, direction = Sort.Direction.DESC) Pageable pageable,
                        Model model){
        model.addAttribute("page", typeService.listType(pageable));
        for (Type type : typeService.listType(pageable)
             ) {
            System.out.println(typeService.listType(pageable).getNumber());
            System.out.println(typeService.listType(pageable).getContent());
        }
        return "admin/types";
    }
